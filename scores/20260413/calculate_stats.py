...
def validate_score_format(score_str):
    ...
    if score_a > 30 or score_b > 30:
        return False, f"比分过高（>30）：'{score_str}'，可能解析错误"
    
    return True, None

def validate_match_logic(score_a, score_b):
    """
    验证比赛逻辑是否合理
    规则：每局必须有一方达到15分获胜
    返回: (是否有效, 错误信息, 警告信息)
    """
    parts_a = score_a.split(':')
    parts_b = score_b.split(':')
    
    score_a_1, score_a_2 = int(parts_a[0]), int(parts_a[1])
    score_b_1, score_b_2 = int(parts_b[0]), int(parts_b[1])
    
    errors = []
    warnings = []
    
    # 检查第一局
    if score_a_1 != 15 and score_a_2 != 15:
        errors.append(f"第一局({score_a})没有一方达到15分")
    
    # 检查第二局
    if score_b_1 != 15 and score_b_2 != 15:
        errors.append(f"第二局({score_b})没有一方达到15分")
    
    # 检查是否有异常高的比分（可能解析错误）
    for score in [score_a_1, score_a_2, score_b_1, score_b_2]:
        if score > 21:
            warnings.append(f"比分{score}较高，可能解析有误")
    
    # 检查是否有比分相同的情况（平局不应该出现）
    if score_a_1 == score_a_2:
        warnings.append(f"第一局比分相同({score_a})，这不应该发生")
    if score_b_1 == score_b_2:
        warnings.append(f"第二局比分相同({score_b})，这不应该发生")
...
def validate_all_data(matches):
    ...
    for i, match in enumerate(matches):
        match_id = f"第{match['round']}轮-场地{match['court']}"
        
        # 检查必填字段
        required_fields = ['round', 'court', 'type', 'team_a', 'team_b', 'score_a', 'score_b']
        ...
        # 验证比分格式
        for score_field in ['score_a', 'score_b']:
        ...
        # 验证比赛逻辑
        if 'score_a' in match and 'score_b' in match:
            is_valid, errors, warnings = validate_match_logic(match['score_a'], match['score_b'])
            if not is_valid:
                for error in errors:
                    report["errors"].append(f"{match_id}: {error}")
                report["valid"] = False
            for warning in warnings:
                report["warnings"].append(f"{match_id}: {warning}")
        
        # 检查是否有备注（需要用户确认的）
        if 'notes' in match and match['notes']:
            report["has_notes"].append({
                "match_id": match_id,
                "notes": match['notes']
            })
...
def format_rankings(rankings):
    """格式化排名表格"""
    lines = []
    lines.append(f"| 排名 | 球员 | 胜局数 | 负局数 | 总局数 | 胜率 |")
    lines.append(f"|------|------|--------|--------|--------|------|")
...
def generate_report(player_stats, stats_by_type, validation_report):
    ...
    lines.append("# 2026年04月13日 球员胜负统计排名")
    lines.append("")
    ...
    if validation_report['has_notes']:
        lines.append("## 📝 需要确认的数据")
        lines.append("")
        for note in validation_report['has_notes']:
            lines.append(f"- **{note['match_id']}**: {note['notes']}")
        lines.append("")
    ...
    lines.append("## 统计说明")
    lines.append("")
    lines.append("- **比赛日期**: 2026年04月13日")
    lines.append("- **赛制**: 15分/局，每场2局")
    lines.append(f"- **总比赛场次**: {validation_report['total_matches']}场")
    lines.append(f"- **总比赛局数**: {validation_report['total_matches'] * 2}局")
    lines.append(f"- **参赛人数**: {len(player_stats)}人")
    lines.append("- **排名规则**: 按胜率从高到低排序，胜率相同者排名并列")
    lines.append("- **数据验证**: 已进行比分格式、比赛逻辑等验证")
    lines.append("")
    lines.append("## 备注")
    lines.append("")
    lines.append("- \"李棋棋\"已纠正为\"李祺祺\"")
